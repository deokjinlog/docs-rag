...
| `/retrieve` (검색+리랭킹, **top_k=3**) | **15,699** | 19,160 | 모델 추론 (CPU 모드) |

† payout p95 240ms·mean 21ms은 첫 호출 커넥션/JIT 워밍업 이상치 — steady-state는 한 자릿수 ms.

**SQL 경로 p50 중앙값 ≈ 6.4ms**, 모두 한 자릿수~10ms 안에서 안정.

### retrieve 지연은 `top_k`에 선형 — 표의 수치를 홑값으로 읽으면 안 된다

리랭커는 `(query, chunk)` 쌍마다 forward를 한 번 돌므로 비용이 **후보 수에 비례**한다.
후보 수 = `top_k × SEARCH_PREFETCH_MULTIPLIER`(=3). 두 지점을 나란히 놓으면 선형이 그대로 보인다:

| 출처 | top_k | 리랭킹 후보 | p50 | 쌍당 |
|---|---|---|---|---|
| 이 벤치 (`bench.py`) | 3 | 9 | 15,699ms | **1.74s** |
| 검색 골든 (`eval_retrieval.py`, trace 실측) | 10 | 30 | 52,600ms | **1.75s** |

쌍당 비용이 1.74 ↔ 1.75s로 일치한다. trace 이력에서도 p50이 3주간(08-13·09-03·09-04) ~50s로
안정적이라 일시적 부하가 아니라 **구조적 바닥**이다. 측정 중 api 컨테이너는 CPU 700%(torch
스레드 8/16코어)를 쓰고 OOM·스왑 압력은 0 — 즉 메모리 문제가 아니라 순수 연산 바운드다.
BGE-reranker-v2-m3는 XLM-R-large(560M)라 CPU fp32 forward가 쌍당 초 단위인 게 정상이다.

함의:
- **`top_k`는 recall 손잡이인 동시에 지연 곱셈기다.** recall@5가 이미 1.00이면 top_k를 키울
  이유가 없다 — 지연만 선형으로 는다.
- 이 바닥을 실제로 낮추는 레버는 top_k도 프리페치 배수도 아닌 **리랭커 배치**다:
  GPU 이전(vLLM과 경합) 또는 경량 리랭커(m3 → base). 둘 다 품질 재측정이 선행 조건.
- **SQL 경로(6.4ms)가 이 비용을 통째로 건너뛴다** — 이 문서가 재려던 payoff의 크기가
  2,500배가 아니라 top_k=10 기준 **8,000배**다.
...
## 부하·동시성 (처리량 QPS + 부하 하 지연)

단일요청 지연이 "한 번에 얼마나 빠른가"라면, 부하 벤치는 "**동시에 얼마나 버티나**"를 잰다.
`bench.py --load`(`make bench-load`)가 SQL 경로에 동시성을 걸어(닫힌-루프: N개 스레드가 마감까지
연속 발사) 처리량과 부하 하 지연 꼬리를 측정한다. 대상은 가장 무거운 SQL(`/payout` — `payout_rule`
조회 + 면책 강제첨부 JOIN). 레벨당 3초, lite/CPU 모드.

| 동시성 | QPS | p50 (ms) | p95 (ms) | p99 (ms) | err |
|---|---|---|---|---|---|
| 1 | 112.8 | 7.8 | 10.6 | 14.1 | 0 |
| 2 | 173.9 | 10.5 | 16.6 | 27.6 | 0 |
| 4 | 178.0 | 21.7 | 29.6 | 35.3 | 0 |
| **8** | **183.1** | 42.6 | 60.1 | 69.9 | 0 |
| 16 | 172.3 | 90.0 | 118.2 | 130.7 | 0 |
| 32 | 178.9 | 172.1 | 243.2 | 266.8 | 0 |

**읽는 법:**
- **처리량은 동시성 4~8에서 ~180 QPS로 포화**(피크 183 @ 8), 이후 평평. 단일 노드·단일 API
  컨테이너의 결정론 SQL 처리량 상한이 이 근방.
- 포화 뒤 동시성을 더 올리면 **QPS는 그대로·지연만 선형 증가**(p50 7.8→172ms) — 초과 요청이 큐에
  쌓이는 것. 과부하 붕괴가 아니라 우아한 포화.
- **에러 0**(전 레벨) — 결정론 계층은 부하에서 실패 꼬리 없이 안정.
- **측정 자체 검증(Little's 법칙, N ≈ QPS×지연)**: c=8 → 183×0.0426≈7.8, c=32 → 178.9×0.172≈30.8.
  관측 동시성과 일치 → 닫힌-루프 포화가 정상 동작, 수치가 내부 일관적.

> 스케일 함의: 포화점이 동시성 ~8이라는 건 **API 워커/커넥션 풀** 상한 신호(현재 단일 컨테이너).
> 수평 확장(워커·레플리카)의 여지는 여기서 읽힌다 — SQL 경로는 무상태 SELECT라 레플리카로 선형
> 확장 가능. RAG(retrieve/answer)는 GPU가 병목이라 확장 곡선이 다르다(별도 측정 지점).
...
- SQL 경로 p95 이상치(단일요청 payout 240ms)는 커넥션 풀 워밍업 — 상시 트래픽에선 사라진다(부하
  벤치의 p95가 60ms대로 안정적인 게 방증). 워밍업 후 재측정은 후속.
- 부하 벤치는 **단일 노드·단일 엔드포인트** — 다중 레플리카 수평 확장 곡선, RAG 경로 부하(GPU 병목),
  혼합 워크로드(SQL+RAG 동시)는 확장 지점.
