...
> **동작 결정 — flag-only.** `risk_level`을 `warnings`로 **노출만** 하고 답변은 그대로 반환한다. 전문가 검토 툴이라 자동 교정보다 *"근거와 함께 플래그"* 가 맞다. 자동 교정(Critic)은 **opt-in · 기본 off** (아래). 개선은 재생성이 아니라 **측정(RAGAS/Recall@k) → 검증기 정밀화**로 간다 — [design-retrospective §1.5](design-retrospective.md).
...
failure type·3축 좌표·hint 생성 상세(opt-in 경로): [grader.py](../src/v1/rag/grader.py) `classify_failure()`/`build_hint()`, 집계 [trace_summary.py](../scripts/trace_summary.py) Section 10.
