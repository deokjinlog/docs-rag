...
**결론**: KB는 `select_profile`에 **제3 프로파일(kb)** 추가 = 요약서 영역 배제 + 보통약관 dedup + 제N장>코드명 특약 2층 분해. 회사미상 경로(2층)·New치아/다이렉트(제N절)와 분리해 무회귀. 4문서 parse 골든(50)+회사미상 적재가 회귀 가드. 검색(청크)은 계속 무관 → 서비스 영향 없음(관계형 SQL 경로만 KB 특약 미개통).
