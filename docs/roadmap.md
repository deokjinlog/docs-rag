...
→ **결론(확정): KB 관계형 개통은 reading-order/파서/단순 재추출로 안 되고, semantic 세그먼테이션(LLM 보조 or 사람-가이드 특약 경계 라벨)이 선결 = 큰 별도 프로젝트.** 검색(청크)은 무관 → **서비스 영향 없음**(관계형 SQL 경로만 KB 특약 미개통). 회사미상 2개사는 이 규모·병리가 없어 복합파서로 적재 완료 — **KB는 1237p 이미지-heavy 특유의 문제로 격리됨.**
