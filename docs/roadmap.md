...
→ **분리 가능한 요약블록이 아니라 1237p OCR 전반에 조-참조/중복이 산재**. 파서·단순 dedup 둘 다 불가 확정. **실 개통 경로 = ①ODL 재추출(이미지-heavy 다단을 조 순서·중복 없이) 또는 ②`.json`의 page number+bounding box로 reading-order 재구성 후 참조/중복 필터 — 둘 다 1237p 규모의 추출계층 프로젝트.** 검색(청크)은 무관 → 서비스 영향 없음(관계형 SQL 경로만 KB 특약 미개통). (회사미상은 이 규모·이미지비중이 없어 복합파서로 적재 완료 — KB PDF 특유 병리.)
