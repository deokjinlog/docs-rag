...
**⚑ 근본 원인(2026-08-20, 파고든 끝 — 파서 아니라 ODL 출력 문제, 재추출 선결)**: 위 5개를 파고들어 **진짜 블로커는 ODL 출력의 엉킴**임을 확정(초기 "깨끗한 2× 중복" 표현은 과했고, 정확히는 아래):
- **조 중복 + 순서 뒤섞임 + 헤딩 아티팩트** — 깨끗한 보통약관 body[25~27%]에서 조매치 75개/고유 53. 시퀀스 `[1,2,2,3,3,4,30,30,5,6,…]`처럼 인접 중복(제2·제3 2×)·순서역행(제30이 제4·제5 사이·제46 조기)·빈 라인 헤딩(본문 없는 `제N조` 마커 다수)이 섞임. 자녀보험 661런의 대부분이 이 엉킴이 만든 false-reset(front-matter는 제외해도 641).
- **인용법령 아님(반증)** — 금융소비자보호법·"보통약관 제N조" 확장 마스킹 = **0 효과**(661→661). 과분할이 인용참조 때문이 아님을 확정.
- **요약서 배제만으론 미미** — kb_body_region([보통약관 제1조 목적 → 트레일링 인용법규정])으로 front-matter 잘라도 143→126·661→641.
→ **KB는 파서 프로파일이 아니라 ODL 추출 산출물의 구조 문제**. 조가 순서 뒤섞이고 중복·빈헤딩으로 나온 입력엔 세그먼트-후-dedup도 순환(특약끼리 제1조 공유 + 순서가 깨져 경계 불명).

**ODL `.json` 트리 조사(2026-08-20, 추출계층 바닥까지)**: 자녀보험 `.json`(24.5MB, `kids` 31,947노드) 직접 분석 —
- **1237페이지 · 이미지 노드 12,101개** = 스캔/이미지-heavy 문서(OCR 산출). 이 규모·이미지비중이 reading-order 엉킴의 근원.
- **조 헤딩이 `list item` 타입**(3447개, `- 제N조(…)`)으로 분류 — heading(438)이 아님. `제1조` 522회·`제2조` 553회·`제3조` 529회 등장.
- **제2조(553) > 제1조(522)** → "특약마다 제1조 1회" 구조와 모순 = **참조("제2조를 따릅니다")+중복 방출**이 카운트를 부풀림. 즉 조 헤딩 매치의 상당수가 실제 조 시작이 아니라 참조/중복.
→ **분리 가능한 요약블록이 아니라 1237p OCR 전반에 조-참조/중복이 산재**. 파서·단순 dedup 둘 다 불가 확정.

**②reading-order 재구성 실측 — 기각(2026-08-20, 골든라이프 6.3MB .json)**: 위 "실 개통 경로 ②"(bbox reading-order 재구성)를 실제로 구현·측정해 **기각**했다:
- **다단 확정 + 순서 복원 됨** — KB는 2단 레이아웃(x0~50 좌단·~380 우단). 노드를 `page → 단(x0 갭) → y 내림차순`(PDF 좌표 y-up이라 내림차순=위→아래)으로 재정렬하니 **보통약관이 뒤죽박죽 .md → 제1→53조 정순으로 복원**(다단 jumbling 가설 실증).
- **그러나 clause 수확은 안 늘어남** — `parse_compound` 실측: 현재 .md(서브계약 143·총조 987·연속서브 116) ≈ 재구성(149·982·111). **reading-order는 병목이 아니었다** — 조는 이미 ~987개 추출되고, 재구성해도 동일.
- **진짜 문제는 semantic 구분** — "제20조"가 페이지마다 다른 제목(청약철회 p75·계약변경 p270·불공정영업 p365…) = 100+ 서브계약이 각자 제1~N조를 가진 정상 복합문서 + 일부 진짜 페이지중복 + 요약/참조. **1237p 안에서 "진짜 특약 조 시작 vs 요약/참조/중복"을 semantic하게 가르는 것**이 남은 난제 — 순서·마스킹·dedup으로 안 됨.
**③재구성 + title 휴리스틱 — 돌파(2026-08-20, "LLM 필요" 결론 정정)**: 위 ②의 "reading-order로는 안 됨"은 절반만 맞았다. 재구성 자체는 clause 수확을 안 늘리지만, **재구성이 특약 제목을 제1조 바로 앞에 놓는다**(예: `###### 1. 장기요양간병비(1~5급)(간편가입)` → `제1조(보험금의 지급사유)`) — 이게 naming의 열쇠였다. 재구성 + **title 휴리스틱**(`###### N. 담보명` 헤딩 + 준용규정 조 보유 판별식으로 요약/용어/조건 배제)으로:
- **골든라이프 85 특약·613조를 정확한 담보명으로 추출**(장기요양간병비·간병지원금 등), 슬기로운 97·710. **LLM 없이 휴리스틱만으로.**
- 운전자·자녀는 title 접미가 달라(("(간편)" 없는 순수 번호담보명 `14. 5대골절진단비`·【갱신계약】) 패턴 튜닝 필요(현 휴리스틱은 과/과소 추출 오감).
→ **정정된 결론: KB는 LLM·재추출·사람라벨이 아니라 `reconstruct_reading_order`(구현·커밋됨) + KB-title 휴리스틱으로 열린다.** 남은 것 = ①title 패턴을 4개 KB 문서군(간편health·운전자·자녀)에 튜닝(bounded) ②적재(docker). 재구성이 다단 문서에서 title을 clause에 붙여주는 게 핵심 enabler. 검색(청크)은 무관 → **서비스 영향 없음**. 회사미상 2개사는 이 병리 없어 이미 적재 완료.

**KB deterministic 라우트 갭 확정(2026-08-20)**: KB clause/ref/면책맵 적재 후 /exclusion 실측 → **matched=False**(KB 면책 데이터는 있으나 라우트 상품해소가 KB 이름형 `1. 장기요양간병비(1~5급)(간편가입)`과 불일치 + coverage_exclusion_map.coverage_name 비어있음). 즉 KB의 payout/coverage/terms/exclusion **SQL 라우트 개통 = 별도 큰 phase**: ①각 라우트 상품해소를 KB 이름형에 맞춤 ②KB 별표 지급표 파싱(payout)·별표3 ICD(coverage)·terms 추출 ③5개사 golden 확장·무회귀(precision-first). KB는 현재 **RAG(검색)+관계형 구조(clause·ref·면책조)까지 개통**, deterministic 값 라우트만 이 phase 대기. **코드-레벨 스코핑(2026-08-20 확인)**: ①`terms_sql.coverage_hint`가 고정 `_COVERAGE_KEYWORDS` 리스트라 KB 담보명(장기요양간병비 등) 미포함→힌트 실패 ②`repository.get_terms(coverage_kw)`가 `WHERE parent_policy_id IS NULL`로 base(보통약관)만 해소→KB 특약(child)은 원천 미해소. → phase 착수 시 공유 해소 로직(coverage_hint 일반화·child 특약 해소용 신 resolver)을 payout/coverage/terms/exclusion 4라우트 공통으로 손봐야 하고, golden_sql_routing 16문항 무회귀 + KB 문항 추가가 게이트(연쇄수정: exclusion_sql·terms_sql·repository·router·골든 동시).
