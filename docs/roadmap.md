...
→ **KB는 파서 프로파일이 아니라 ODL 추출 산출물의 구조 문제**. 조가 순서 뒤섞이고 중복·빈헤딩으로 나온 입력엔 세그먼트-후-dedup도 순환(특약끼리 제1조 공유 + 순서가 깨져 경계 불명). **선결 = ODL 재추출 조사**(다단/요약 레이아웃을 조 순서·중복 없이 뽑는 설정) 또는 ODL `.json` 레이아웃 트리로 reading-order 복원. 검색(청크)은 무관 → 서비스 영향 없음(관계형 SQL 경로만 KB 특약 미개통). **파서 착수 전에 추출계층부터가 순서.** ODL `.json` 레이아웃 트리 존재(자녀보험 24.5MB, `kids` 재귀구조=페이지·블록 reading-order) → 여기서 요약/전문 블록 분리·중복 소스 규명이 착수점. (회사미상은 이 엉킴이 없어 복합파서로 적재 완료 — KB PDF 특유 레이아웃 병리.)
