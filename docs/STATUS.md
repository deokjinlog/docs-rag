...
- **🔧 복합약관 다절 파서 착수(검증된 사이드카)** — `parse_clauses` 메인경로는 복합약관을 첫 서브계약(보통약관)만 파싱(단조 break). KB(특약 다절)·회사미상 2건 모두 복합약관(상해질병 216× 특별약관 `## 특별약관` 섹션·수술비=LIG파워업연금·KB 제N장). **조-리셋 불변식**(특약은 각각 제1조부터 재시작)으로 `detect_subcontracts`(감지)+`parse_compound`(개별 파싱)를 **메인경로 미변경 사이드카**로 구현(회귀 0). 실측 상해질병 **42조 → 42서브계약 231조**(특약 제목 정확)·KB 자녀보험 특약 540개 감지. `check_parsing`이 물려 "과소파싱 7조" → "복합약관: 보통약관 39조 + 특약 99개" 정밀 진단 + `--subs`로 특약 분해를 눈으로. 유닛 6. **남은 것 = 적재 배선**(`ingest_compound`이 split_sections 대신 parse_compound로 → DB, 스택 소관) → 그 후 parse 골든·SQL 경로 특약 확장.
