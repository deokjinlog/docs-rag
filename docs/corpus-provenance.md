...
(`scripts/batch_scale.py`, 코드열거 21000~23500 + md5 dedup + pymupdf 고속 파싱). 두 축을 분리해 잰다 —
...
> 데이터: `data/_batch_scale/` (300 PDF·0.6GB, gitignore) + `_metrics.json`. 스케일 데모용 처리 코퍼스라
> 골든 미부착(정밀도 골든은 5~6개사 유지). 축2 개선을 원하면 **300개를 ODL로 재추출**(느림·GPU)해야
> 조/특약 일반화를 잴 수 있음 — 처리량은 이미 증명됐고, 구조 파싱은 ODL 통과가 전제.
