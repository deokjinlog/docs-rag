...
검색은 이미 강했고(표준 하이브리드+rerank로 충분), 그 위에 얹은 자기교정 루프(CRAG·Critic)는 **p95 지연을 2배로 만들면서 품질을 거의 못 올렸다.** 필요했던 건 복잡한 에이전트가 아니라, **RAGAS로 자생하는 평가 루프** — 측정 → (컨텍스트 품질 개선 또는 파인튜닝) → 재측정 — 이었다. CRAG·Critic 같은 구체 설계는 그 루프가 "검색이 진짜 병목"이라고 말할 때만 켜면 된다(평소엔 꺼둠).
...
하나라도 명확히 통과 → **필수(메인 경로)**, 애매하면 **유예(기본 꺼짐 + 트리거)**.

> **판정 방법 (정직)** — 전 레이어를 구현한 뒤 **1회 운영 측정(trace 27건)** 으로 관찰 → 분류. 즉 *build → 측정 → 값 못 하는 건 유예.* 단 통제된 A/B가 아니라 관찰 애블레이션이고, 코드를 지운 게 아니라 **Critic만 기본 off · 나머지는 기본 꺼짐으로 재분류**. n=27·하루치·합성셋이라 "확정"이 아니라 *"이 데이터에선 이렇다"*.

| | 항목 | 근거 |
|---|---|---|
| **필수** (메인) | 수집·적재(상태코드 재처리) · 하이브리드 검색+rerank · grounding 프롬프트+LLM · lean 구조 검증(0ms) | 성립 조건이거나 실측 고ROI (rerank top-1 0.85 · 검증 0.5ms) |
| **유예** (기본 꺼짐+트리거) | Adaptive 라우팅 · CRAG · Critic · 12-섹션 trace · 4층 가드레일 | 실측 이득 미검증 / 현 단계 불필요 (§1 상세) |
| **설계만** (미구현) | 평가 자동화 루프(§5) · 파인튜닝([roadmap](roadmap.md)) | 측정·트래픽이 필요하다고 할 때 |
...
**그래서 "단순하지만 견고한" 최소 세트는:** 좋은 수집·적재(상태코드 재처리) + 하이브리드 검색·rerank + grounding 프롬프트 + **lean trace** + **RAGAS 평가 루프**. 나머지(라우팅·CRAG·Critic·풀 trace·가드레일)는 **측정·트래픽이 필요하다고 할 때 켠다**. 견고함은 레이어 수가 아니라 *(1) 수집·적재 재처리 (2) 검증 신호를 항상 남기는 것 (3) 평가로 회귀를 잡는 것* 에서 온다.
...
문헌이 말하는 정당화 조건: **noisy·adversarial 검색, open-domain 웹스케일 코퍼스, long-form 생성, multi-hop·문서간 종합.** Self-CRAG의 큰 이득(PopQA/Bio/PubHealth/ARC에서 +19 / +14.9 / +36.6 / +8.1pt)은 정확히 이런 하드 오픈도메인에서 나온다. 규칙: **단발 팩트 질의엔 static RAG, 다단 추론·교차종합엔 에이전트 루프 — 그것도 매 쿼리 실행이 아니라 게이트 뒤에서.** 우리의 **닫힌 고품질 약관 코퍼스는 오버레이가 불필요한 케이스**다. 필요해지면 그때 켠다.
...
flowchart LR
    ...
    D -.검색이 진짜 병목일 때만.-> A["agentic 켬<br/>CRAG / Critic"]
...
| 검색이 반복적으로 부족 | 진짜 retrieval 병목 | 이때만 **CRAG 켬** |
...
| **복합약관 = 사이드카** | `detect_subcontracts`·`parse_compound`·`sections_for_ingest`를 메인 파싱경로 미변경으로 — 회귀 0(회사미상 적재), KB는 ODL 추출 문제로 증명해 파서 미착수 | §7 |
| **오적재 차단·revert** | `extract_terms` 청약철회 오추출을 precision 대조가 잡음 → 정규식 땜질은 혼재효과라 revert(ground-truth 골든 선행) | §7 |
| **지연 = 구간 분해 후 레버** | ~22s를 trace 분해(rerank 18.5s/83%) → CPU CrossEncoder 비용 확정, GPU 토글(`make retrieve-gpu`)로 대응 | §7 |

## 7. 회사 넘어 확장 — 복합약관·측정 규율 (2026-08, 5개사 코퍼스)

코퍼스를 5개 회사·13,080청크로 넓히며 나온 세 판단. 공통 축 = **측정·조사가 "무엇을 만들지·안 만들지"를 정한다.**

- **복합약관: 사이드카로 짓고, 될 것/안 될 것을 갈랐다.** 보통약관 + 특약 수십 개인 복합약관을 **조-리셋 불변식**(특약은 각각 제1조부터 재시작)으로 분해 — **메인 파싱경로 미변경 사이드카**라 회귀 0(New치아·다이렉트 `split_sections` 동일 반환 검증). 회사미상 2개사는 42조 → 231조 특약을 복원해 실 DB 적재(product 41→92·특약 49). **KB는 파고든 끝에 ODL 추출 산출물의 조 중복·순서뒤섞임이 진짜 블로커임을 증명**(1237p 이미지-heavy·조=list item·인용법령 마스킹 0효과로 반증) → **파서 프로파일을 짜는 헛수고를 사전 차단.** 판단: *엉킨 입력엔 파서가 아니라 추출계층이 선결. 근본원인을 증명하고 접는다.*
- **측정-먼저가 실 오적재를 막았다.** 회사미상 terms 적재를 시도하다 `extract_terms`가 청약철회를 오추출(수술비 실제 15일인데 반환기일 "3일"을 뭄)함을 precision 대조로 검출. 급히 정규식을 고쳤으나 다른 문서서 혼재효과(상해질병 15→None)라 **깨끗한 win이 아니라 revert.** 판단: *검증 없는 적재는 오답 데이터. 정규식 땜질보다 ground-truth 골든이 선행.*
- **지연은 분해로 병목을 짚었다.** 로컬 ~22s/쿼리를 trace로 분해 → **rerank 18.5s(83%)** 가 CPU CrossEncoder(560M) 자체 비용(이미 배치·재로딩 버그 아님). 레버 = GPU(lite 모드서 8GB가 놈) → `make retrieve-gpu` 토글(기존 `ingest-gpu` 미러). 판단: *추측 최적화 전에 구간 분해. 정책(리랭커 CPU)은 vLLM 경합 회피가 이유라 lite엔 성립 안 함.*
...
- 2026-08-20 · §7 추가 — 회사 넘어 5개사 확장의 세 판단(복합약관 사이드카+KB 추출증명 / 측정이 잡은 오적재·revert / 지연 구간분해→GPU 토글). 결정 로그 3행 추가.
