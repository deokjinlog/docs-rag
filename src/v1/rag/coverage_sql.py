...
# 질의 담보 힌트(병명 키워드 → 담보). '암'은 넓어 마지막(제자리암·경계성·KB 담보 먼저 매칭).
# KB(암진단비·갑상선암·기타피부암)는 다이렉트(암진단자금)와 담보명이 달라 스코프(get_ranges를
# 브랜드 base로) + 이 힌트로 구분. 구체적 담보명이 일반 '암'보다 먼저 와야 오매칭 방지.
_COVERAGE_HINT = [
    ("제자리암", "제자리암진단자금"),
    ("경계성종양", "경계성종양진단자금"),
    ("경계성", "경계성종양진단자금"),
    ("기타피부암", "기타피부암"),     # KB 유사암 소분류
    ("갑상선암", "갑상선암"),          # KB 유사암 소분류(C73)
    ("암진단비", "암진단비"),          # KB 일반암 담보
    ("암", "암진단자금"),              # 다이렉트/일반 폴백
]
