...
- **상태**: 관계형 테이블(`db/schema_insurance.sql`: product·clause·clause_ref·annex·annex_row·coverage_exclusion_map·payout_rule) **실 DB 적재 완료**(로컬 postgres named volume, 재클론 시 재적재) — product 41·clause 307·clause_ref 421·annex 19·annex_row 136·coverage_exclusion_map 25·**payout_rule 94(4상품; 다이렉트 12는 LLM폴백 stub, `LLM_BASE_URL` 설정 후 재적재)**. 적재기: `load_clauses/load_exclusions/load_annexes/load_payout.py --load`(DATABASE_URL=localhost:5433). **남은 것 = src/v1 router의 3경로(SQL/RAG/fetch) 배선(B5)** — SQL 질의로직은 `scripts/query_payout.py`(골든 5/5, 현재 in-memory 대역)로 검증됨, 실서비스는 payout_rule SELECT로 교체해 router에 배선. 골든은 `data/eval/golden_*.jsonl`(force-add, data/ gitignore 예외).
