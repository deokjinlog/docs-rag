...
- **상태**: 관계형 테이블(`db/schema_insurance.sql`: product·clause·clause_ref·annex·annex_row·coverage_exclusion_map·payout_rule) **실 DB 적재 완료**(로컬 postgres named volume, 재클론 시 재적재) — product 41·clause 307·clause_ref 421·annex 19·annex_row 136·coverage_exclusion_map 25·**payout_rule 94(4상품; 다이렉트 12는 LLM폴백 stub, `LLM_BASE_URL` 설정 후 재적재)**. 적재기: `load_clauses/load_exclusions/load_annexes/load_payout.py --load`(DATABASE_URL=localhost:5433). **SQL 경로 서빙 배선됨(사이드카)** — 모듈 `src/v1/rag/payout_sql.py`(select_payout·format_payout, 순수) + `PayoutRepository.get_rules()`(payout_rule SELECT) + **엔드포인트 `POST /payout`**(결정론 답, miss면 matched=false→RAG 폴백). /answer 미변경. **남은 것 = /answer 자동 라우팅 통합**(질의 유형 감지→SQL/RAG 자동 분기, B5). 골든 5/5(실 DB·in-memory), 통합테스트 test_payout_sql_integration(엔드포인트 포함). 골든은 `data/eval/golden_*.jsonl`(force-add, data/ gitignore 예외).
