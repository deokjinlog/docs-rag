...
>
> 그 판정을 사람이 내리는 화면이 **Inspector** 다(아래 섹션) — 원본 페이지를 렌더해
> 그 페이지에서 나온 청크와 나란히 놓고, 색인에 못 닿은 줄을 글자까지 보여준다.
...
게이트는 전부 숫자다. 숫자는 *무엇이 얼마나 어긋났는지*는 말하지만 **그게 문제인지**는 말하지
못한다. R10 이 커버리지 FAIL 이었을 때 열어보니 누락의 대부분이 목차 점선이었다. 그 판정은
사람만 할 수 있고, 사람이 하려면 화면이 있어야 한다. `make inspect` → `http://localhost:8002/inspector`

```
문서 목록      청크·페이지판별·V1 판정 한 줄에      (?dup=1 로 중복군까지)
문서 뷰어      왼쪽 원본 페이지 · 오른쪽 그 페이지 청크 · 색인 도달률과 못 닿은 줄
```

**색인 도달률** — 페이지의 각 줄이 그 페이지 청크에 들어 있나. `kept`(통째로 있다) /
`split`(구간 여러 개로 다 덮인다 = 흩어졌을 뿐 손실 아님) / `missing`(못 덮는 구멍 — 그 글자를
보여준다). 비교 대상이 ODL 트리가 아니라 **색인**인 이유는, 질문이 "ODL 이 읽었나"가 아니라
*"검색이 찾을 수 있나"* 라서다.

> **실데이터가 판정기를 네 번 교정했다.** 전부 정상 문서를 손실로 고발하던 오탐이다 —
> ①조 제목은 `content` 가 아니라 `heading_path` 컬럼에 있다 ②색인은 `<최초계약의 경우>` 를
> `&lt;…&gt;` 로 저장한다 ③PDF 의 `「국민건강보험법」` 이 색인엔 `"국민건강보험법"` 이다
> (`preprocess.py` 정규화) ④ODL 의 페이지 귀속이 PyMuPDF 와 한 줄씩 어긋난다. 넷을 고치자
> R10 p.45 가 **0.85 → 0.983**. 남은 누락은 머리글 한 줄이고, **R10 의 FAIL 은 조판이다**
> (V1 기준선에 달아둔 "본문 손실 여부는 후속" 이 이로써 닫혔다).

**중복은 24.6%인데, 지우면 안 된다.** 코퍼스 8,850청크 중 2,180이 중복이고(검색 골든의
`dup@10` 평균 2.7=27%과 독립적으로 일치) 문서별로 0%~81.3%로 갈린다. 그런데 화면이 보여준 건
겉보기 중복의 정체였다:

| 사례 | 겉보기 | 실제 |
|---|---|---|
| R05 p.15 지급표 2개 | 같은 모양의 표가 두 번 | **상해 원인 / 질병 원인**으로 값이 다르다(레진 50% vs 0/25/50%). 조건 라벨은 별도 청크에 있고 표 청크엔 없다 |
| SCALE_21012 81.3% | 파싱 폭주처럼 보임 | 1~4종이 각자 보통약관을 통째로 가짐. `제2조(청약의 철회)`가 종마다 글자까지 같다 |

둘 다 **구분자가 heading_path 에만** 있다. 그래서 중복 처리는 색인 삭제가 아니라 **회수 시
접기**(대표 1개 + 나머지 접기)여야 한다 — 지우면 "3종의 청약철회"에 답할 수 없다. 표 축
recall@1 이 조문보다 낮은 구조적 이유도 여기 있다: **표 청크에 조건이 안 붙어 있다.**
