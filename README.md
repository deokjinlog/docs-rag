...
**상태**: 관계형 테이블 **실 DB 적재 완료**(product 41·clause 307·payout_rule 94/4상품) + **3경로 라우팅 서빙 중**. `/answer`가 "얼마·지급률" 질의를 **자동으로 SQL 경로**로 보내 결정론 답(LLM 미호출, `route.strategy="sql"`)하고, 담보만 겹치는 "언제 지급·정의" 해석 질의는 RAG로 — amount 게이트 + 규칙 매칭 2중 안전(precision-first). 별도 `POST /payout` 사이드카도 제공. 로직 [`payout_sql.py`](src/v1/rag/payout_sql.py)(골든 5/5 실 DB). 방법론은 [eval-and-golden.md](docs/eval-and-golden.md), 도메인은 [domain-model.md](docs/domain-model.md).
