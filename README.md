...
약관·법령·매뉴얼 같은 **한국어 구조화 PDF**를 등록하면 — 자동으로 추출·OCR·청킹·임베딩해서 인덱싱하고, **하이브리드 검색 + Rerank + LLM 답변**까지 처리하는 파이프라인. 답변이 인용한 조항·숫자가 검색된 문서에 실제로 있는지 **0ms 근거 확인**을 붙여, 어긋나면 플래그한다(자동 교정 없이 전문가 검토용). 자기 교정(CRAG·Critic)도 만들었지만 **측정해보니 값을 못 해서 기본은 꺼두고 필요할 때만 켜도록** 뺐다. 도메인 비종속 — 라우팅 정규식/프롬프트만 바꾸면 재사용된다.

> 차별점은 화려한 기능이 아니라 **판단**이다. 이 repo는 완성된 제품이라기보다 *"이렇게 설계했고, 무엇을 왜 넣고 뺐는지"* 를 정직하게 남긴 기록에 가깝다 — 복잡한 레이어를 다 만들어 본 뒤 값을 못 하는 건 걷어냈다(기본 꺼둠). 판단 근거는 [설계 회고](docs/design-retrospective.md).
...
> 시스템 구성은 [architecture.md](docs/architecture.md), 서빙 분기(CRAG·Critic는 기본 꺼짐)는 [pipeline.md](docs/pipeline.md).

## 핵심 특징

방향은 **단순하지만 견고하게** — 품질의 핵심(문서 처리 · 검색 · 검증)에 집중하고, 복잡한 레이어는 만들어 두되 **측정해서 정말 필요하다고 나올 때만 켠다**(평소엔 꺼둠).
...
- **공짜 근거 확인** — 답변이 인용한 조항·숫자가 검색된 문서에 있는지 정규식으로 대조(0ms) → 없으면 **함께 플래그**. 자동 교정 대신 전문가 검토 (현재는 retrieval-gap 탐지에 가까움 — [회고](docs/design-retrospective.md))
- **측정 기반 개선** — 평가문항(gold set)을 만들어 RAGAS·retrieval 지표로 측정 → 병목(검색/생성)을 진단해 그 축만 개선
- **정직한 설계 기록** — 라우팅·CRAG·Critic·풀 trace·가드레일은 만들어 두되 **기본은 꺼두고, 측정으로 필요성이 확인될 때만 켠다**. 무엇을 왜 넣고 뺐는지 [설계 회고](docs/design-retrospective.md)에 공개
...
**라우팅 → 하이브리드 검색 → Rerank → LLM 생성 → 0ms 근거 확인(플래그)** 순. 근거 밖 참조가 있어도 답은 그대로 반환하고 **경고만** 붙인다(전문가 검토용). CRAG 재검색·Critic 재생성은 **기본 꺼짐 — 필요할 때만 켬** — 단계별 상세는 [pipeline.md](docs/pipeline.md).
...
- 복잡한 레이어(Adaptive 라우팅 · CRAG · Critic 재생성 · 12-섹션 trace · 4층 가드레일)는 **만들어 두되 기본은 꺼두고, 트래픽·측정이 필요하다고 할 때만 켠다**. 초기 측정상 대부분 현 단계엔 불필요했다.
- 근거 확인은 조항·수치의 **존재**만 본다 — 의미 반전("보장하지 아니한다" vs "보장된다")은 못 잡는다. 검증된 한국어 NLI judge가 나오면 `semantic_judge` 슬롯으로 확장.
